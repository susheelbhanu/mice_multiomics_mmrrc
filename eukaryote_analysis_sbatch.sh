...
SMK_JOBS=6
# snakemake file
SMK_SMK="eukaryote_analysis_Snakefile"
# config file
SMK_CONFIG="eukaryote_analysis_config.yaml" # USER INPUT REQUIRED
# slurm config file
...
# run the pipeline
snakemake -s ${SMK_SMK} --cores ${SMK_CORES} --local-cores 1 --jobs ${SMK_JOBS} --configfile ${SMK_CONFIG} --use-conda --conda-prefix ${CONDA_PREFIX}/pipeline \
