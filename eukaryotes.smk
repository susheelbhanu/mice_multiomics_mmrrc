...
rule merge_cov:
    input:
        cov=os.path.join(COV_DIR, "{sample}/run1/Stats/mg/annotation/mg.gene_depth.avg")
    output:
        os.path.join(RESULTS_DIR, "euk_tax_cov/{sample}_eukaryotes.txt")
    params:
        tax=os.path.join(DATA_DIR, "taxonomy_estimation/{sample}-estimated-taxonomy.out")
    log:
    ...
    run:
        ...
        # keeping only rows that have at least 60% 'max_pid'
        filt_euks=euks.query("max_pid >=60")
...
rule euk_abundances:
    ...
    message:
        "Merging all eukaryote abundances for mice MG-MT samples"
    run:
