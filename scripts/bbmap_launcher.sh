...
IMP_JOBS=2
# IMP config file
IMP_CONFIG="bbmap_config.yaml" # USER INPUT REQUIRED
# slurm config file
...
# run the pipeline
snakemake -s bbmap_snakefile -rp --jobs ${IMP_JOBS} --local-cores 1 --configfile ${IMP_CONFIG} \
--use-conda --conda-prefix ${CONDA_PREFIX}/IMP --cluster-config ${IMP_SLURM} --cluster "${IMP_CLUSTER}" -n

# snakemake -s snakefile -rp --jobs ${IMP_JOBS} --configfile ${IMP_CONFIG} \
# --use-conda --cluster-config ${IMP_SLURM} --cluster "${IMP_CLUSTER}"
